...
## 重大更新说明
### 2021-05-06
集成 WebPack，探索小程序工程化体系，思路如下：<br>
src 目录作为源码目录，利用 WebPack 通过 Babel 对 JS 文件进行处理，其余文件类型使用 CopyPlugin 插件进行复制转移。

存在的问题:
1. 由于 JS 文件经过 Babel 编译，所以最终展示的代码跟最初编写的有一些出入，在断点调试的时候不太友好。这部分之后会看看有没有优化解决的方式。
